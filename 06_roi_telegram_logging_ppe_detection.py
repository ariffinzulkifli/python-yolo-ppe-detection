"""
YOLO PPE Detection with ROI, Alerts, Telegram and SQLite Logging
...
# Telegram notification cooldown (seconds)
...
class TelegramNotifier:
    """Handle Telegram notifications with async support"""
    ...
    def send_photo(self, image, caption):
        ...
        try:
            ...
            # Convert numpy array to JPEG bytes with optimization
    ...
    def send_photo_async(self, image, caption):
        """Send photo asynchronously (non-blocking)"""
        if not self.enabled:
            return

        # Submit to thread pool and return immediately
        self.executor.submit(self.send_photo, image.copy(), caption)
...
class PPEDetectionApp(QMainWindow):
    ...
    def send_telegram_notification(self, violation_message):
        """Send Telegram notification with photo (async)"""
        ...
        # Prepare message with local timezone
        timestamp = current_time.strftime("%Y-%m-%d %H:%M:%S")

        message = f"""
<b>⚠️ PPE VIOLATION ALERT</b>

<b>Time:</b> {timestamp} (Asia/Kuala_Lumpur)
<b>Zone:</b> {CAMERA_ZONE_NAME}
<b>Violation:</b> {violation_message}

<b>Session Statistics:</b>
• Total People: {self.session_total_people}
• Compliant: {self.session_compliant}
• Violations: {self.session_violations}

<b>Action Required:</b>
Please ensure all personnel wear proper PPE equipment in the designated area.

<i>Automated safety notification from PPE Detection System</i>
"""

        # Send photo asynchronously (non-blocking)
        if self.current_frame is not None:
            self.telegram.send_photo_async(self.current_frame, message)
            self.alert_log.append(f"[{timestamp}] Telegram notification sent (async)")
